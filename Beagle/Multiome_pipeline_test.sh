...
for item in ${sample_list[@]}; do
	...
	#Comment out the following lines based on whether you are running on a subset of the data
	input_bam="./possorted_genome_bam_subset.bam" #this version is applicable to the sample file approach
	...
	GATK="/storage/home/hcoda1/0/cnaughton7/anaconda3/envs/beagle-env/bin/gatk"
	PICARD="java -jar /storage/home/hcoda1/0/cnaughton7/p-ggibson3-0/Software/picard/build/libs/picard.jar"
	...
	#SamToFastq
	#Changed from gatk to picard
	$PICARD SamToFastq --INPUT $reverted_bam  --VALIDATION_STRINGENCY SILENT --FASTQ $fastq1 --SECOND_END_FASTQ $fastq2

	#Run this once
	#StarGenerateReferences - previously run so commenting out here
	#STAR --runMode genomeGenerate --genomeDir $genome_dir --genomeFastaFiles $ref_fasta --sjdbGTFfile ../../Data/Reference/gencode.v39.annotation.gtf --runThreadN 11 --genomeSAsparseD 3 --genomeSAindexNbases 12

	#StarAlign
	# Needed for 
	STAR --genomeDir $genome_dir --runThreadN 11 --readFilesCommand "gunzip -c" --outSAMtype BAM SortedByCoordinate --twopassMode Basic --limitBAMsortRAM 45000000000 --outFileNamePrefix $outFileNamePrefix --readFilesIn $fastq1 #second fastq was empty so removing $fastq2

	#MergeBamAlignment
	# Not working on WGS files
	#Changed from gatk to picard 
	$PICARD MergeBamAlignment --REFERENCE_SEQUENCE $ref_fasta --UNMAPPED_BAM $reverted_bam --ALIGNED_BAM $aligned_bam --OUTPUT $merged_bam --INCLUDE_SECONDARY_ALIGNMENTS false --PAIRED_RUN False --VALIDATION_STRINGENCY SILENT

	#MarkDuplicates

	$PICARD MarkDuplicates --INPUT $merged_bam --OUTPUT $dupes --CREATE_INDEX true --VALIDATION_STRINGENCY SILENT --METRICS_FILE $dupe_metrics
